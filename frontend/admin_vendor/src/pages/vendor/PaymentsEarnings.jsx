...
const PaymentsEarnings = () => {
    ...
    useEffect(() => {
        const fetchTransactionHistory = async () => {
            try {
                ...
                // Make sure response structure is valid
    ...
    return (
        <>
                    ...
                    <div className="overflow-x-auto rounded-lg max-h-[90vh] overflow-y-auto mt-10 ">
                        <table className="min-w-full  divide-gray-200">
                            <thead className="bg-gray-50 sticky top-0 z-10">
                                <tr>
                                    <th className="px-4 py-2 text-left text-sm font-medium text-gray-700 w-30">Date</th>
                            ...
                            <tbody className="bg-white  divide-gray-200">
                                {sales.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={10} className="px-4 py-2 text-gray-500 text-center">
                                            No sales found.
                                        </td>
                                    </tr>
                                )}
                            ...
                            <tbody className="bg-white  divide-gray-200">
                                {payouts.length > 0 ? (
                                    payouts.map((payout, i) => (
                                        <tr key={i} >
                                            <td className="px-4 py-2">{payout?.payout_id || "N/A"}</td>
                                            <td className="px-4 py-2">₹{payout?.amount || 0}</td>
                                            <td className="px-4 py-2">{payout?.status || "-"}</td>
                                            <td className="px-4 py-2">{payout?.date || "-"}</td>
                                            <td className="px-4 py-2">{payout?.description || "-"}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
            ...
            <div className='bg-[#ECECF0] px-6 py-10 rounded-2xl mt-13'>
                ...
                <div className='min-w-full bg-white rounded-lg shadow text-sm'>
                    ...
                    <div className="overflow-x-auto  max-h-100 scrollbar-none px-5 py-2">
                        <table className="w-full ">
                            <thead>
                                <tr>
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">
                                        <button
                                    ...
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">Transaction ID</th>
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">Type</th>
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">Product</th>
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">Status</th>
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">Order ID</th>
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">Amount (INR)</th>
                                    <th className="text-left py-2 px-4 font-medium text-gray-700 text-sm">Description</th>
                                </tr>
                            </thead>
                            <tbody>
                                {transactionData.length > 0 ? (
                                    transactionData.map((transaction, index) => (
                                        <tr key={index} className="border-b border-gray-50 hover:bg-gray-25">
                                            ...
                                            <td className="py-2 px-4 text-gray-700">{transaction.type}</td>
                                            <td className="py-2 px-4 text-gray-700">{transaction.product}</td>
                                            ...
                                            <td className="py-2 px-4 text-gray-700">{transaction.orderId}</td>
                                            <td className="py-2 px-4 text-gray-700 font-medium">{transaction.amount}</td>
